...
(function(){
  ...
  function statusHTML(p,compact=false){const a=availability(p);if(a.chance===100)return'';return `<span class="fxAvailability ${a.kind}" data-chance="${a.chance}" title="${a.chance===0?'Will not play':'Might play'} · ${a.chance}%${a.news?' · '+a.news:''}">!${compact?'':' '+a.chance+'%'}</span>`}
  function decorate(){
    document.querySelectorAll('.player').forEach(row=>{
      const p=playerByName(row.querySelector('.marketPlayerInfo b')?.textContent);if(!p)return;
      const a=availability(p),old=row.querySelector('.fxAvailability');
      if(a.chance===100){old?.remove();return}
      if(!old||Number(old.dataset.chance)!==a.chance){old?.remove();const h=statusHTML(p);if(h)row.querySelector('.marketPlayerInfo b')?.insertAdjacentHTML('afterend',h)}
    });
    document.querySelectorAll('.teamPlayerCard').forEach(card=>{
      ...
      const a=availability(p),old=top.querySelector('.fxAvailability');
      if(a.chance===100)old?.remove();else if(!old||Number(old.dataset.chance)!==a.chance){old?.remove();const h=statusHTML(p,true);if(h)top.insertAdjacentHTML('beforeend',h)}
      const v=top.querySelector('.viceBadge');if(p.vice&&!v)top.insertAdjacentHTML('beforeend','<span class="viceBadge" title="Vice captain">V</span>');else if(!p.vice)v?.remove();
    });
  }
  window.setViceCaptain=id=>{if(typeof S==='undefined')return;const p=S.find(x=>x.id===Number(id));if(!p)return;if(p.cap)return alert('Your captain and vice captain must be different players.');S.forEach(x=>x.vice=false);p.vice=true;if(typeof save==='function')save();if(typeof pitch==='function')pitch();setTimeout(decorate,0)};
  const originalCap=window.cap;if(originalCap)window.cap=i=>{if(typeof S!=='undefined'&&S[i]?.vice)S[i].vice=false;originalCap(i);setTimeout(decorate,0)};
  ...
  window.addEventListener('load',()=>{setTimeout(decorate,700);setInterval(decorate,1500);document.querySelectorAll('.tab').forEach(b=>b.addEventListener('click',()=>setTimeout(decorate,50)))});
})();