...
const SPECIAL_TRANSFER_UNTIL=Date.parse('2026-09-18T12:51:48+02:00');
...
app.put('/api/account/squad',async(req,res)=>{try{if(!await requireUser(req,res))return;const state=transferWindowState();if(!state.open)return res.status(423).json({error:state.message});const h=tokenHash(req),next=(req.body?.squad||[]).slice(0,15),old=await rpc('fx_get_squad',{p_token_hash:h})||[],oldIds=new Set(old.map(x=>Number(x.player_id))),nextIds=new Set(next.map(x=>Number(x.id))),incoming=[...nextIds].filter(id=>!oldIds.has(id)).length;if(state.penalty&&incoming>0)await rpc('fx_record_transfer_penalty',{p_token_hash:h,p_gameweek:state.targetGw,p_transfers:incoming});await rpc('fx_save_squad',{p_token_hash:h,p_squad:next});try{trackTransferActivity(await rpc('fx_public_squads',{}))}catch{}res.json({ok:true,gameweek:state.targetGw,transfers_in:incoming,penalty:state.penalty?incoming*4:0})}catch(e){res.status(500).json({error:String(e.message)})}});
